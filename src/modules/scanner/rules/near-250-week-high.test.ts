...
function buildWeeklyCandles(close: number) {
  return Array.from({ length: 250 }, (_, index) => ({
    ...
    high: index === 120 ? 1000 : 700,
    close: index === 249 ? close : 700,
  }));
}

describe("near 250-week high scanner rule", () => {
  it("matches when the latest weekly close is above 85% of the 250-week high", () => {
    const result = calculateNear250WeekHighScan(buildWeeklyCandles(900), 250);

    expect(result?.matched).toBe(true);
    expect(result?.metrics.highestHigh250).toBe(1000);
  ...
  it("matches when the latest weekly close equals 85% of the 250-week high", () => {
  ...
  it("does not match below 85% of the 250-week high", () => {
